		...
		Case Else
			If CanUpdateField(strFieldName) Then
				If IsRequiredField(strFieldName) And IsNull(RestoreNull(Request(strFieldName))) Then
					...
					Exit Function
				End If				
				rsPicktPick(strFieldName) = RestoreNull(Request(strFieldName))
			End If
	End Select
End Function
...
Function UpdateField(strFieldName)
	...
	Select Case rsPicktPick(strFieldName).Type
		...
		Case Else
			...
			If Not IsEqual(ConvertToString(rsPicktPick(strFieldName)), RestoreNull(Request(strFieldName))) Then
				If CanUpdateField(strFieldName) Then						
					If IsRequiredField(strFieldName) And IsNull(RestoreNull(Request(strFieldName))) Then
						RaiseError errValueRequired, strFieldName
						UpdateField = False
...
Else
	Response.Redirect "PickForm.asp?FormMode=Edit"
End If
...
Select Case strDataAction
	...
	Case "List View"
		...
		Response.Redirect "PickList.asp"
	...
	Case "Filter"
		...
		On Error Resume Next
		Session("rsPicktPick_Filter") = ""
		Session("rsPicktPick_FilterDisplay") = ""
		Session("rsPicktPick_Recordset").Filter = ""
		Response.Redirect "PickForm.asp?FormMode=" & strDataAction
	...
	Case "Find"
		...
		Session("rsPicktPick_PageSize") = 1 'So we don't do standard page conversion
		Session("rsPicktPick_AbsolutePage") = CLng(Request("Bookmark"))
		Response.Redirect "PickForm.asp"
	...
	Case "Apply"
		...
		strWhere = ""
		strWhereDisplay = ""
		FilterField "PickID", Null
		FilterField "MemberID", avarPicktMember
		FilterField "Pick", Null
		FilterField "Comment", Null
		FilterField "Date", Null
        
		' Filter the recordset
		If strWhere <> "" Then
			Session("rsPicktPick_Filter") = strWhere
			Session("rsPicktPick_FilterDisplay") = strWhereDisplay
			Session("rsPicktPick_AbsolutePage") = 1
		Else
			Session("rsPicktPick_Filter") = ""
			Session("rsPicktPick_FilterDisplay") = ""
		...
		If Err.Number = 0 Then Response.Redirect "PickForm.asp"
	...
	Case "Insert"
		...
		Do
			If Not InsertField("PickID") Then Exit Do
			If Not InsertField("MemberID") Then Exit Do
			If Not InsertField("Pick") Then Exit Do
		...
		If Err.Number <> 0 Then
			If rsPicktPick.EditMode Then rsPicktPick.CancelUpdate
		Else
			If IsEmpty(Session("rsPicktPick_AbsolutePage")) Or Session("rsPicktPick_AbsolutePage") = 0 Then
				Session("rsPicktPick_AbsolutePage") = 1
			End If
			' Requery static cursor so inserted record is visible
			If rsPicktPick.CursorType = adOpenStatic Then rsPicktPick.Requery
			Session("rsPicktPick_Status") = "Record has been inserted"
		End If
	...
	Case "Update"
		...
		Set rsPicktPick = Session("rsPicktPick_Recordset")
		If rsPicktPick.EOF and rsPicktPick.BOF Then Response.Redirect "PickForm.asp"
		
		Do

			If Not UpdateField("PickID") Then Exit Do
			If Not UpdateField("MemberID") Then Exit Do
			If Not UpdateField("Pick") Then Exit Do
			If Not UpdateField("Comment") Then Exit Do
			If Not UpdateField("Date") Then Exit Do

			If rsPicktPick.EditMode Then rsPicktPick.Update
			Exit Do
		Loop
	...
	Case "Delete"
		...
		If Err.Number = 0 Then
			...
			If rsPicktPick.EOF Then
				Session("rsPicktPick_AbsolutePage") = Session("rsPicktPick_AbsolutePage") - 1				
				If rsPicktPick.BOF And rsPicktPick.EOF Then rsPicktPick.Requery
			End If
		End If
   ...
   If Err Then %>
	...
	<table WIDTH="100%" CELLSPACING="0" CELLPADDING="0" BORDER="0">
		...
		<tr>
			...
			Select Case strDataAction
				Case "Insert"
					Response.Write("Unable to insert the record into tPick.")
				Case "Update"
					Response.Write("Unable to post the updated record to tPick.")
				Case "Delete"
					Response.Write("Unable to delete the record from tPick.")
			End Select
			%>
			</b></font>
			</td>
...
<!-- Action feedback -->
	...
	<head>
		...
		<meta NAME="keywords" CONTENT="Microsoft DataForm, tPick Form">
		<title>tPick Form</title>
	</head>
	<basefont FACE="Arial, Helvetica, sans-serif">
	<body>
	<table WIDTH="100%" CELLSPACING="0" CELLPADDING="0" BORDER="0">
		...
		<tr>
			...
			Select Case strDataAction
				Case "Insert"
					Response.Write("The following record has been inserted into PicktPick.")
				Case "Update"
					Response.Write("The following updated record has been posted to PicktPick.")
				Case "Delete"
					Response.Write("The following record has been deleted from PicktPick.")
			End Select
			%>
			</font>
			</td>
	...
	<table WIDTH="100%" CELLSPACING="1" CELLPADDING="2" BORDER="0">
		...
		<%
			FeedbackField "PickID", "PickID", Null
			FeedbackField "MemberID", "MemberID", avarPicktMember
			FeedbackField "Pick", "Pick", Null
