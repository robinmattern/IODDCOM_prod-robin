...
Sub ShowField(strFieldName, strLabel, blnIdentity, avarLookup)
	...
	strFieldValue = ""
	If strFormMode = "Edit" Then strFieldValue = RTrim(rsPredictiontPrediction(strFieldName))
	...
	intInputSize = rsPredictiontPrediction(strFieldName).DefinedSize + 2
	...
	Select Case rsPredictiontPrediction(strFieldName).Type
		...
		Case Else
			Dim nType 
			nType=rsPredictiontPrediction(strFieldName).Type
...
If Not IsEmpty(Request("rsPredictiontPrediction_PagingMove")) Then
    strPagingMove = Trim(Request("rsPredictiontPrediction_PagingMove"))
End If
%>
...
<% Response.Write "<FORM ACTION=""PredictionAction.asp"" METHOD=""POST"">" %>
<table WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="0">
		...
		<td ALIGN="Right" BGCOLOR="Green" VALIGN="MIDDLE" WIDTH="100%">
			...
			Select Case strFormMode
				Case "Edit"	
					...
					<% If Session("rsPredictiontPrediction_Filter") <> "" Then %>
	...
	<tr>
		<td BGCOLOR="#FFFFCC" COLSPAN="3">
			<font SIZE="-1">&nbsp;&nbsp;
			<%
			If Not IsEmpty(Session("rsPredictiontPrediction_Status")) And Session("rsPredictiontPrediction_Status") <>"" Then
				Response.Write Session("rsPredictiontPrediction_Status")
				Session("rsPredictiontPrediction_Status") = ""
			Else
				Select Case strFormMode
					Case "Edit"
						If IsEmpty(Session("rsPredictiontPrediction_Filter")) Then
							Response.Write "Current Filter: None"
						Else
							If Session("rsPredictiontPrediction_Filter") <> "" Then
								Response.Write "Current Filter: " & Session("rsPredictiontPrediction_FilterDisplay")
							Else
								Response.Write "Current Filter: None"
							End If
						End If
...
If strFormMode = "Edit" Then
	...
	ShowField "PredictionID", "PredictionID", True, Null
	ShowField "MemberID", "MemberID", False, avarPredictiontMember
	ShowField "Prediction", "Prediction", False, Null
