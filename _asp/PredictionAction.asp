...
Function IsRequiredField(strFieldName)
	...
	If (rsPredictiontPrediction(strFieldName).Attributes And adFldIsNullable) = 0 Then 
...
Function CanUpdateField(strFieldName)
	...
	CanUpdateField = True
	If (rsPredictiontPrediction(strFieldName).Attributes And intUpdatable) = False Then
		...
		Case Else
			If CanUpdateField(strFieldName) Then
				If IsRequiredField(strFieldName) And IsNull(RestoreNull(Request(strFieldName))) Then
					...
					Exit Function
				End If				
				rsPredictiontPrediction(strFieldName) = RestoreNull(Request(strFieldName))
			End If
	End Select
End Function
...
Function UpdateField(strFieldName)
	...
	Select Case rsPredictiontPrediction(strFieldName).Type
		...
		Case Else
			...
			If Not IsEqual(ConvertToString(rsPredictiontPrediction(strFieldName)), RestoreNull(Request(strFieldName))) Then
				If CanUpdateField(strFieldName) Then						
					If IsRequiredField(strFieldName) And IsNull(RestoreNull(Request(strFieldName))) Then
						RaiseError errValueRequired, strFieldName
						UpdateField = False
...
Else
	Response.Redirect "PredictionForm.asp?FormMode=Edit"
End If
...
Select Case strDataAction
	...
	Case "List View"
		...
		Response.Redirect "PredictionList.asp"
	...
	Case "Filter"
		...
		On Error Resume Next
		Session("rsPredictiontPrediction_Filter") = ""
		Session("rsPredictiontPrediction_FilterDisplay") = ""
		Session("rsPredictiontPrediction_Recordset").Filter = ""
		Response.Redirect "PredictionForm.asp?FormMode=" & strDataAction
	...
	Case "Find"
		...
		Session("rsPredictiontPrediction_PageSize") = 1 'So we don't do standard page conversion
		Session("rsPredictiontPrediction_AbsolutePage") = CLng(Request("Bookmark"))
		Response.Redirect "PredictionForm.asp"
	...
	Case "Apply"
		...
		strWhere = ""
		strWhereDisplay = ""
		FilterField "PredictionID", Null
		FilterField "MemberID", avarPredictiontMember
		FilterField "Prediction", Null
		FilterField "Date", Null
        
		' Filter the recordset
		If strWhere <> "" Then
			Session("rsPredictiontPrediction_Filter") = strWhere
			Session("rsPredictiontPrediction_FilterDisplay") = strWhereDisplay
			Session("rsPredictiontPrediction_AbsolutePage") = 1
		Else
			Session("rsPredictiontPrediction_Filter") = ""
			Session("rsPredictiontPrediction_FilterDisplay") = ""
		...
		If Err.Number = 0 Then Response.Redirect "PredictionForm.asp"
	...
	Case "Insert"
		...
		Set rsPredictiontPrediction = Session("rsPredictiontPrediction_Recordset")
		rsPredictiontPrediction.AddNew
		
		Do
			If Not InsertField("PredictionID") Then Exit Do
			If Not InsertField("MemberID") Then Exit Do
			If Not InsertField("Prediction") Then Exit Do
			If Not InsertField("Date") Then Exit Do

			rsPredictiontPrediction.Update
			Exit Do
		Loop

		If Err.Number <> 0 Then
			If rsPredictiontPrediction.EditMode Then rsPredictiontPrediction.CancelUpdate
		Else
			If IsEmpty(Session("rsPredictiontPrediction_AbsolutePage")) Or Session("rsPredictiontPrediction_AbsolutePage") = 0 Then
				Session("rsPredictiontPrediction_AbsolutePage") = 1
			End If
			' Requery static cursor so inserted record is visible
			If rsPredictiontPrediction.CursorType = adOpenStatic Then rsPredictiontPrediction.Requery
			Session("rsPredictiontPrediction_Status") = "Record has been inserted"
		End If
	...
	Case "Update"
		...
		Set rsPredictiontPrediction = Session("rsPredictiontPrediction_Recordset")
		If rsPredictiontPrediction.EOF and rsPredictiontPrediction.BOF Then Response.Redirect "PredictionForm.asp"
		
		Do

			If Not UpdateField("PredictionID") Then Exit Do
			If Not UpdateField("MemberID") Then Exit Do
			If Not UpdateField("Prediction") Then Exit Do
			If Not UpdateField("Date") Then Exit Do

			If rsPredictiontPrediction.EditMode Then rsPredictiontPrediction.Update
			Exit Do
		Loop
	...
	Case "Delete"
		...
		rsPredictiontPrediction.Delete
		...
		If Err.Number = 0 Then
			...
			If rsPredictiontPrediction.EOF Then
				Session("rsPredictiontPrediction_AbsolutePage") = Session("rsPredictiontPrediction_AbsolutePage") - 1				
				If rsPredictiontPrediction.BOF And rsPredictiontPrediction.EOF Then rsPredictiontPrediction.Requery
			End If
		End If
...
'<!----------------------------- Error Handler --------------------------------->
   ...
   If Err Then %>
	...
	<head>
		...
		<meta NAME="keywords" CONTENT="Microsoft Data Form, tPrediction Form">
		<title>tPrediction Form</title>
	...
	<table WIDTH="100%" CELLSPACING="0" CELLPADDING="0" BORDER="0">
		...
		<tr>
			...
			Select Case strDataAction
				Case "Insert"
					Response.Write("Unable to insert the record into tPrediction.")
				Case "Update"
					Response.Write("Unable to post the updated record to tPrediction.")
				Case "Delete"
					Response.Write("Unable to delete the record from tPrediction.")
			End Select
			%>
			</b></font>
			</td>
...
<!-- Action feedback -->
	...
	<head>
		...
		<meta NAME="keywords" CONTENT="Microsoft DataForm, tPrediction Form">
		<title>tPrediction Form</title>
	</head>
	<basefont FACE="Arial, Helvetica, sans-serif">
	<body>
	<table WIDTH="100%" CELLSPACING="0" CELLPADDING="0" BORDER="0">
		...
		<tr>
			...
			Select Case strDataAction
				Case "Insert"
					Response.Write("The following record has been inserted into PredictiontPrediction.")
				Case "Update"
					Response.Write("The following updated record has been posted to PredictiontPrediction.")
				Case "Delete"
					Response.Write("The following record has been deleted from PredictiontPrediction.")
			End Select
			%>
			</font>
			</td>
	...
	<table WIDTH="100%" CELLSPACING="1" CELLPADDING="2" BORDER="0">
		...
		<%
			FeedbackField "PredictionID", "PredictionID", Null
			FeedbackField "MemberID", "MemberID", avarPredictiontMember
			FeedbackField "Prediction", "Prediction", Null
...
<% 
End If 
Set rsPredictiontPrediction = Nothing
%>
