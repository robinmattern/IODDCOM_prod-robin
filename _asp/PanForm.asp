...
Sub ShowField(strFieldName, strLabel, blnIdentity, avarLookup)
	...
	strFieldValue = ""
	If strFormMode = "Edit" Then strFieldValue = RTrim(rsPantPan(strFieldName))
	...
	intInputSize = rsPantPan(strFieldName).DefinedSize + 2
	...
	Select Case rsPantPan(strFieldName).Type
		...
		Case Else
			Dim nType 
			nType=rsPantPan(strFieldName).Type
...
If Not IsEmpty(Request("rsPantPan_PagingMove")) Then
    strPagingMove = Trim(Request("rsPantPan_PagingMove"))
End If
%>
...
<% Response.Write "<FORM ACTION=""PanAction.asp"" METHOD=""POST"">" %>
<table WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="0">
		...
		<td ALIGN="Right" BGCOLOR="Green" VALIGN="MIDDLE" WIDTH="100%">
			...
			Select Case strFormMode
				Case "Edit"	
					...
					<% If Session("rsPantPan_Filter") <> "" Then %>
	...
	<tr>
		<td BGCOLOR="#FFFFCC" COLSPAN="3">
			<font SIZE="-1">&nbsp;&nbsp;
			<%
			If Not IsEmpty(Session("rsPantPan_Status")) And Session("rsPantPan_Status") <>"" Then
				Response.Write Session("rsPantPan_Status")
				Session("rsPantPan_Status") = ""
			Else
				Select Case strFormMode
					Case "Edit"
						If IsEmpty(Session("rsPantPan_Filter")) Then
							Response.Write "Current Filter: None"
						Else
							If Session("rsPantPan_Filter") <> "" Then
								Response.Write "Current Filter: " & Session("rsPantPan_FilterDisplay")
							Else
								Response.Write "Current Filter: None"
							End If
						End If
...
If strFormMode = "Edit" Then
	...
	ShowField "PanID", "PanID", True, Null
	ShowField "MemberID", "MemberID", False, avarPantMember
	ShowField "Pan", "Pan", False, Null
